...
tripadvisor, X_train01, X_test01 = load_data()
...
def build_model(X_train, X_test):
    ...
    return vectorizer, tfidf_train, tfidf_test, similarity_matrix


vectorizer, tfidf_train, tfidf_test, similarity_matrix_m01 = build_model(
    X_train01,
    X_test01
)
...
def recommend_similar_place_tfidf(idplace_query, top_k=5, top_n_words=5):
    ...
    for i, idx in enumerate(top_indices_sim):
        ...
        with cols[i]:
            ...
            similar_id = int(X_train01.iloc[idx]['idplace'])
...
if st.session_state.run:
    ...
    rec_ids = recommend_similar_place_tfidf(
