...
st.title("🎯 TF-IDF Retrival Recommendation System")
...
def load_data():
    ...
    X_train03 = pd.concat([part1, part2], ignore_index=True)
    #X_train03 = pd.read_csv("data/X_train.csv")
    X_test03 = pd.read_csv("data/X_test.csv")

    return tripadvisor, X_train03, X_test03


tripadvisor, X_train03, X_test03 = load_data()
...
vectorizer, tfidf_train, tfidf_test, similarity_matrix_m03 = build_model(
    X_train03,
    X_test03,
    tripadvisor
)
...
def recommend_similar_place_tfidf_retrieval(idplace_query, top_k=5, top_n_words=5):

    if idplace_query not in X_test03['idplace'].values:
        st.warning("ID not found")
        return []

    X_test_reset = X_test03.reset_index(drop=True)

    query_pos = X_test_reset.index[
        X_test_reset['idplace'] == idplace_query
    ][0]

    sims = similarity_matrix_m03[query_pos]
    ...
    for i, idx in enumerate(top_indices_sim):
        ...
        with cols[i]:
            ...
            similar_id = int(X_train03.iloc[idx]['idplace'])
...
if st.session_state.run:
    ...
    rec_ids = recommend_similar_place_tfidf_retrieval(
